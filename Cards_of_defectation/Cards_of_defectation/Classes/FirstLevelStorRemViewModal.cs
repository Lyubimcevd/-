...
{
    public class FirstLevelStorRemViewModal: INotifyPropertyChanged
    {       
        ...
        public FirstLevelStorRemViewModal()
        {
        ...
        public FirstLevelStorRemViewModal(int id)
        {
            parent_row = new Row_in_kart_defect();
            parent_row.Par = id;
            if (id != 0)
                parent_row.Nom_zay = Convert.ToInt32(Server.InitServer().DataBase("test1")
                       .ExecuteCommand("select nom_zay from kart_defect where id = " + parent_row.Par)[0]);
            zav_izgot_list = Server.InitServer().DataBase("cvodka")
                        .ExecuteCommand("select distinct Ltrim(rtrim(zakazchi_naim)) from zakazchi_naim");
            is_change = false;
        }
        public FirstLevelStorRemViewModal(RowDefectViewModal row)
        {
            parent_row = row.Save;
